...
# --------------------
# Name extraction (NFL CSV rows -> use every text column)
# --------------------
...
# --------------------
# Custom typed team builder (any team names)
# --------------------
def build_custom_team_df(team_name: str, aliases_text: str = "") -> pd.DataFrame:
    names = []
    if team_name and team_name.strip():
        names.append({"source": "custom", "name": team_name.strip()})
    if aliases_text and aliases_text.strip():
        for part in re.split(r"[;,\\n]", str(aliases_text)):
            p = part.strip()
            if p:
                names.append({"source": "alias", "name": p})

    # dedupe by cleaned key
    by_key = {}
    for item in names:
        key = _clean_text(item["name"])
        if not key:
    ...
    rows = []
    for it in deduped:
        sc = gematria_scores(it["name"])
        rows.append({"source": it["source"], "name": it["name"], **sc})
    df = pd.DataFrame(rows)
    if not df.empty:
        df = df.sort_values(by=["name"]).reset_index(drop=True)
    return df
...
home_df = pd.DataFrame(columns=["name","ordinal","reduction","reverse_ordinal","reverse_reduction"])
away_df = pd.DataFrame(columns=["name","ordinal","reduction","reverse_ordinal","reverse_reduction"])

if source == "NFL CSV":
    # selection style within NFL
...
# ---- Custom typed branch ----
    ...
    with colA:
        ...
        home_aliases = st.text_area("Home aliases / variants (optional, comma or semicolon)", value="", height=80, placeholder="e.g., Alabama; Crimson Tide; Bama; UA; Alabama Tide")
    with colB:
        away_text = st.text_input("Away team name", value="", placeholder="e.g., Georgia Bulldogs")
        away_aliases = st.text_area("Away aliases / variants (optional, comma or semicolon)", value="", height=80, placeholder="e.g., Georgia; Bulldogs; UGA; Georgia Dawgs")
    home_df = build_custom_team_df(home_text, home_aliases)
    away_df = build_custom_team_df(away_text, away_aliases)

# ---- Common inputs ----
game_date = st.date_input("Game date", value=date.today())
home_qb = st.text_input("Home QB (optional)", value="", placeholder="e.g., Bryce Young")
away_qb = st.text_input("Away QB (optional)", value="", placeholder="e.g., Jalen Hurts")
venue = st.text_input("Venue (City / Stadium)", value="", placeholder="e.g., Bryant–Denny Stadium, Tuscaloosa, AL")
